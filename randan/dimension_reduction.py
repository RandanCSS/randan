...
class CA:
    ...
    def fit(
        ...
        if data_type.lower() == "raw":
            ...
            raise ValueError(
                """Unknown data type. Possible values: 'raw' or 'crosstab'"""
            )

        while (
            len(self.crosstab.loc[self.crosstab.var(axis=1) == 0.0, :]) > 0
            or len(self.crosstab.loc[:, self.crosstab.var() == 0.0].T) > 0
        ):
            ...
            print(
                "One or more categories were removed from analysis as they have zero variance."
            )

        max_inertia = min(len(self.crosstab.columns), len(self.crosstab.index)) - 1
        ...
        try:
            self.column_profiles.columns = self.column_profiles.columns.add_categories(
                "Mean column profile"
            )
        ...
        try:
            self.row_profiles.index = self.row_profiles.index.add_categories(
                "Mean row profile"
            )
        ...
        self.expected_frequencies = pd.DataFrame(
            np.outer(
                self.correspondence_matrix.sum(), self.correspondence_matrix.sum(axis=1)
            ).T,
        ...
        self.standardized_residuals = (
            self.correspondence_matrix - self.expected_frequencies
        ) / np.sqrt(self.expected_frequencies)

        self.inertia_total = (self.standardized_residuals**2).sum().sum()

        self.U, self.singular_values, self.V = np.linalg.svd(
            self.standardized_residuals, full_matrices=True
        )

        self.inertia_by_dimensions = pd.DataFrame(
            self.singular_values**2,
            columns=["Inertia"],
            index=[f"Dimension {i + 1}" for i in range(len(self.singular_values))],
        )
        self.inertia_by_dimensions["Inertia (%)"] = (
            self.inertia_by_dimensions["Inertia"] / self.inertia_total * 100
        )
        self.inertia_by_dimensions = self.inertia_by_dimensions.iloc[
            : self.n_dimensions, :
        ]
        self.inertia_by_dimensions.loc["Total"] = self.inertia_by_dimensions.sum()

        self.standard_row_coordinates = (np.diag(self.row_mass.values**-0.5) @ self.U)[
            :, :max_inertia
        ]
        self.standard_column_coordinates = (
            np.diag(self.column_mass.values**-0.5) @ self.V[:max_inertia, :].T
        )

        self.principal_row_coordinates = self.standard_row_coordinates @ np.diag(
            self.singular_values[:max_inertia]
        )
        self.principal_column_coordinates = (
            np.diag(self.column_mass.values**-0.5)
            @ self.V[:max_inertia, :].T
            @ np.diag(self.singular_values[:max_inertia])
        )

        self.row_inertia_components = (
            np.diag(self.row_mass.values) @ self.principal_row_coordinates**2
        )
        self.column_inertia_components = (
            np.diag(self.column_mass.values) @ self.principal_column_coordinates**2
        )

        self.contribution_of_rows_to_inertia_of_dimensions = (
            self.row_inertia_components
            / self.row_inertia_components.sum(axis=0)[np.newaxis, :]
        )
        self.contribution_of_columns_to_inertia_of_dimensions = (
            self.column_inertia_components
            / self.column_inertia_components.sum(axis=0)[np.newaxis, :]
        )
        ...
        self.contribution_of_dimensions_to_inertia_of_rows = (
            self.row_inertia_components / self.inertia_of_rows[:, np.newaxis]
        )
        self.contribution_of_dimensions_to_inertia_of_columns = (
            self.column_inertia_components / self.inertia_of_columns[:, np.newaxis]
        )

        self.communalities_rows = self.contribution_of_dimensions_to_inertia_of_rows[
            :, : self.n_dimensions
        ].sum(axis=1)
        self.communalities_columns = (
            self.contribution_of_dimensions_to_inertia_of_columns[
                :, : self.n_dimensions
            ].sum(axis=1)
        )

        self.component_loadings_rows = (
            self.contribution_of_dimensions_to_inertia_of_rows** 0.5
            * np.sign(self.principal_row_coordinates)
        )
        self.component_loadings_rows = pd.DataFrame(
            self.component_loadings_rows[:, : self.n_dimensions],
            columns=[f"Dimension {i + 1}" for i in range(self.n_dimensions)],
            index=self.correspondence_matrix.index,
        )
        self.component_loadings_columns = (
            self.contribution_of_dimensions_to_inertia_of_columns** 0.5
            * np.sign(self.principal_column_coordinates)
        )
        self.component_loadings_columns = pd.DataFrame(
            self.component_loadings_columns[:, : self.n_dimensions],
            columns=[f"Dimension {i + 1}" for i in range(self.n_dimensions)],
        ...
        if plot_dimensions:
            ...
            if self.n_dimensions == 1:
                ...
                if self.n_dimensions > 2:
                    ...
                    print(
                        f"To get more plots, use [model].plot_dimensions((1, {self.n_dimensions}))"
                    )
    ...
    def show_results(self, n_decimals):
        ...
        display(
            self.inertia_by_dimensions.style.format(
                None, na_rep="", precision=n_decimals
    ...
    def summary(
        ...
        if display_coordinates == True:
            for i in range(self.n_dimensions):
                summary_table[f"Coordinates (dim. {i + 1})"] = list(
                    self.principal_row_coordinates[:, i]
                ) + list(self.principal_column_coordinates[:, i])

        summary_table["Inertia (%)"] = list(
            self.inertia_of_rows / self.inertia_total * 100
        ) + list(self.inertia_of_columns / self.inertia_total * 100)

        if display_contributions == True:
            for i in range(self.n_dimensions):
                summary_table[
                    f"Contribution of points to inertia of dimensions (dim. {i + 1})"
                ] = list(
                    self.contribution_of_rows_to_inertia_of_dimensions[:, i]
                ) + list(self.contribution_of_columns_to_inertia_of_dimensions[:, i])

            for i in range(self.n_dimensions):
                summary_table[
                    f"Contribution of dimensions to inertia of points (dim. {i + 1})"
                ] = list(
                    self.contribution_of_dimensions_to_inertia_of_rows[:, i]
                ) + list(self.contribution_of_dimensions_to_inertia_of_columns[:, i])

        if display_component_loadings == True:
            for i in range(self.n_dimensions):
                summary_table[f"Component loadings of points (dim. {i + 1})"] = list(
                    self.component_loadings_rows.iloc[:, i]
                ) + list(self.component_loadings_columns.iloc[:, i])

        summary_table[f"Communalities ({self.n_dimensions} dims)"] = list(
            self.communalities_rows
        ) + list(self.communalities_columns)
    ...
    def show_components(self, axis=0, n_indicators=None):
        ...
        column_categories = [
            str(category) + "__COLUMN" for category in self.column_categories
        ]
        ...
        if axis == 0 or axis == "row":
            ...
            raise ValueError(
                "Unknown axis. Possible values: 'row' (0) and 'column' (1)"
            )
        ...
        for i in range(2, n_components + 2):
            positive_loadings_index = list(
                summary.loc[categories_to_show]
                .iloc[:, i]
                .sort_values(ascending=False)[:n_indicators]
                .index
            )
            positive_loadings_values = list(
                summary.loc[categories_to_show]
                .iloc[:, i]
                .sort_values(ascending=False)[:n_indicators]
            )
            positive_loadings = list(
                zip(positive_loadings_index, positive_loadings_values)
            )
            positive_loadings = [
                str(loading) + "*" + index for index, loading in positive_loadings
            ]
            components[f"Component {i - 1} (positive loadings)"] = positive_loadings

            negative_loadings_index = list(
                summary.loc[categories_to_show]
                .iloc[:, i]
                .sort_values(ascending=True)[:n_indicators]
                .index
            )
            negative_loadings_values = list(
                summary.loc[categories_to_show]
                .iloc[:, i]
                .sort_values(ascending=True)[:n_indicators]
            )
            negative_loadings = list(
                zip(negative_loadings_index, negative_loadings_values)
            )
            negative_loadings = [
                str(loading) + "*" + index for index, loading in negative_loadings
            ]
        ...
        for component in components.columns:
            components[component] = (
                components[component]
                .str.replace("__ROW", "")
                .str.replace("__COLUMN", "")
            )

        return components

    def plot_dimensions(
        self, dimensions_range=(1, 2), size=100, n_symbols=20, save=False
    ):
        """
        ...
        dimension_combinations = list(
            combinations(range(dimension_low, dimension_high + 1), 2)
        )
        ...
        for plot in range(n_plots):
            ...
            x1, y1 = (
                self.principal_row_coordinates[:, current_combination_x],
                self.principal_row_coordinates[:, current_combination_y],
            )
            x2, y2 = (
                self.principal_column_coordinates[:, current_combination_x],
                self.principal_column_coordinates[:, current_combination_y],
            )
            ...
            for i, txt in enumerate(self.row_categories):
                plt.annotate(
                    txt[:n_symbols] + "...", (x1[i] - x_delta, y1[i] + y_delta)
                )

            for i, txt in enumerate(self.column_categories):
                plt.annotate(
                    txt[:n_symbols] + "...", (x2[i] - x_delta, y2[i] + y_delta)
                )
            ...
            plt.fill_between(xticks, y_min, 0, where=xticks <= 0, alpha=0.2)
            plt.fill_between(xticks, y_min, 0, where=xticks >= 0, alpha=0.2)
            plt.fill_between(xticks, 0, y_max, where=xticks <= 0, alpha=0.2)
            plt.fill_between(xticks, 0, y_max, where=xticks >= 0, alpha=0.2)
...
class Rotator:
    ...
    def _varimax(self, X=None):
        ...
        loadings = pd.DataFrame(
            X.T, columns=self.loadings_.columns, index=self.loadings_.index
        )
    ...
    def _promax(self):
        ...
        # convert loadings matrix to data frame
        loadings = pd.DataFrame(
            z, columns=self.loadings_.columns, index=self.loadings_.index
        )
    ...
    def fit_transform(self):
        ...
        if method == "varimax":
            (new_loadings, new_rotation_mtx) = self._varimax()

        elif method == "promax":
            (new_loadings, new_rotation_mtx, phi) = self._promax()
...
class PCA:  # первичен
    ...
    def __init__(self, n_components="Kaiser", rotation=None, kappa=4):
        if str(n_components).lower() == "kaiser":
            ...
            raise ValueError("""Invalid number of components was passed.
        ...
        if rotation is not None:
            if rotation.lower() not in possible_rotations:
                raise ValueError(
                    f"Invalid type of rotation was passed. Possible values: {', '.join(possible_rotations)}."
                )
            else:
    ...
    def fit(
        ...
        self.correlation_matrix = pd.DataFrame(
            np.corrcoef(init_df.T), columns=init_df.columns, index=init_df.columns
        )
        self.correlation_matrix.index.name = "Variable"
        self._eigenvectors, self._eigenvalues, self._eigenvectors_t = np.linalg.svd(
            self.correlation_matrix
        )
        self._eigenvalues, self._eigenvectors = PCA._sort_sign(
            self._eigenvalues, self._eigenvectors
        )

        # max_n_components solution
        # start = time.time()
        init_component_loadings = PCA._loadings(
            self._eigenvalues, self._eigenvectors, self.variables, self._PCs_init_list
        )
        ...
        if self.n_components_criterion == "Kaiser":
            self.n_components = len(
                self.explained_variance[
                    self.explained_variance.iloc[:, 0].astype("float") >= 1
                ]
            )
        #             if print_decision:
        ...
        self.explained_variance_total = self.explained_variance.loc[
            self.n_components, "Cumulative %"
        ]
        ...
        if self.rotation is not None:
            if self.n_components > 1:
                self.component_loadings_rotated, self.structure_matrix = (
                ...
                print(
                    "Rotation could not be performed because number of dimensions is 1."
                )
                self.rotation = None
        ...
        self.communalities_and_loadings = pd.concat(
            [self.communalities, necessary_loadings], axis=1
        )
        ...
        if self.rotation in ["natural collinearity", "promax"]:
            ...
            self.correlation_matrix_components = pd.DataFrame(
                np.corrcoef(self._transformed_data_rotated.T),
    ...
    def show_results(self, print_decision=True, n_decimals=3):
        ...
        if self.rotation != "natural collinearity":
            ...
            if print_decision:
                if self.n_components_criterion == "Kaiser":
                    print(
                        f"The number of selected components by Kaiser criterion: {self.n_components}"
                    )
                elif (
                    self.n_components_criterion
                    == "Inflection point (based on scree plot)"
                ):
                    print(
                        f"The number of selected components by an inflection point: {self.n_components}"
                    )
                print("------------------\n")
        ...
        if self.rotation != "natural collinearity":
            ...
            display(
                self.get_explained_variance(scree_plot=True)
                .style.format(None, na_rep="", precision=n_decimals)
                .set_caption("methods .get_explained_variance() and .scree_plot()")
            )
            print(
                f"The model explains {round(self.explained_variance_total, n_decimals)}% of variance."
            )
            print("------------------\n")
        ...
        display(
            self.communalities_and_loadings.style.format(
                None, na_rep="", precision=n_decimals
            ).set_caption("attribute .communalities_and_loadings")
        )
        print(
            f"The minimum communality is {round(self.communalities['Communality'].min(), n_decimals)}."
        )
        if self.rotation in ["natural collinearity", "promax"]:
            print("Components' correlation")
            display(
                self.correlation_matrix_components.style.format(
                    None, na_rep="", precision=n_decimals
    ...
    def transform(self, data, standardize=True, add_to_data=False):
        ...
        if self.rotation is not None and self.rotation == "natural collinearity":
            return PCA._nat_collinearity(
                data,
                self.n_components,
                show=False,
                add_to_data=add_to_data,
                loadings_matrix=True,
            )[3]
        else:
        ...
        component_scores_df = pd.DataFrame(
            np.dot(df, weights), index=df.index, columns=loadings_df.columns
        )
        if standardize:
            component_scores_df = (
                component_scores_df - component_scores_df.mean()
            ) / component_scores_df.std()
    ...
    def _rotation_matrices(
        self, data, n_components, vars_list, PCs_list, loadings_, show
    ):
        if self.rotation == "natural collinearity":
            matrices = PCA._nat_collinearity(
                data, n_components, show=show, add_to_data=False, loadings_matrix=True
            )
            ...
            if self.rotation == "varimax":
                ...
                structure_matrix = pd.DataFrame(
                    structure_matrix, columns=loadings.columns, index=loadings.index
                )
    ...
    def _inflection_point(explained_variance, tol=0.05):
        percent_df = pd.DataFrame(
            columns=["Num", "Percent"], index=range(1, len(explained_variance) + 1)
        )
        ...
        for i in range(
            len(percent_df) - 1, 0, -1
        ):  # running from the right point on the graph to the left
            results = smf.ols(
                "Percent~Num", data=percent_df.loc[i : len(percent_df), :]
            ).fit()
            a = results.rsquared
            results = smf.ols(
                "Percent~Num", data=percent_df.loc[i - 1 : len(percent_df), :]
            ).fit()
            b = results.rsquared
            inflection_df.loc[i, "delta"] = a - b

        inflection_point = (
            inflection_df[inflection_df["delta"] == inflection_df["delta"].max()].index[
                0
            ]
            - 1
        )
    ...
    @staticmethod
    def _nat_collinearity(
        data, n_components, show=True, add_to_data=False, loadings_matrix=False
    ):
        vrmx_model = PCA(n_components=n_components, rotation="varimax").fit(
            data, print_decision=False, show_results=False
        )
        ...
        for i_v in dist_2["Variables"]:
            ...
            component_loadings[f"PC{i}_natcol"] = model.component_loadings[
                "PC1"
            ]  # component loadings -- МОГУТ ВЫВОДИТЬСЯ ПО СПЕЦ.ЗАПРОСУ
            eigenvalue += model.get_explained_variance(
                scree_plot=show if i < 5 else False
            )["Eigenvalue"][1]
            component_scores[f"PC{i}_natcol"] = model.transform(data[i_v])
        pc_corr = pd.DataFrame(
            np.corrcoef(component_scores.T),
            columns=component_scores.columns,
            index=component_scores.columns,
        ...
        if show:
            print(
                f"The total eigenvalue explained by Natural collinearity PCA (i.e., {len(dist_2)} independent PCA-models) = {round(eigenvalue, 3)}",
    ...
    def get_communalities(self, min_max=True):
        ...
        if min_max:
            print(
                f"The min communality: {round(communalities['Communality'].min(), 3)}, the max communality: {round(communalities['Communality'].max(), 3)}"
            )
    ...
    def scree_plot(self, annotate_bars=True, annotate_current=True):
        ...
        if annotate_current:
            acc_sum_by_n_components = [
                explained_variance.iloc[self.n_components - 1, 2]
            ] * len(pc_num)
            plt.plot(
                pc_num,
                acc_sum_by_n_components,
    ...
    def variables_components_distribution(self, threshold=0.5, delta=0.1, show=True):
        """
        ...
        if self.n_components < 2:
            print(
                "The distributions would not be shown because number of dimensions is 1."
            )
        else:
            loadings_df = self._necessary_loadings.copy()
            PCs_list = self._PCs_final_list.copy()
            ...
            for PC in PCs_list:
                PCs_through_vars[PC] = PCs_through_vars[PC].apply(
                    lambda loading: "+" if abs(loading) >= threshold else ""
                )
            ...
            for PC in PCs_list:
                vars_candidates_plus = list(
                    vars_through_PCs[PC][vars_through_PCs[PC] >= threshold].index
                )
                vars_candidates_minus = list(
                    vars_through_PCs[PC][vars_through_PCs[PC] <= -threshold].index
                )
                # if vars_candidates != []:
                vars_through_PCs_list.append(
                    [PC, vars_candidates_plus, vars_candidates_minus]
                )
            vars_through_PCs = pd.DataFrame(vars_through_PCs_list)
            vars_through_PCs.columns = [
                "Component",
                "Variables (positive side)",
                "Variables (negative side)",
            ]
            ...
            try:
                ...
                for PC in PCs_list:
                    loadings_df_3[PC] = loadings_df_3[PC].apply(
                        lambda loading: abs(loading)
                ...
                for var in loadings_df_3_1.columns:
                    ...
                    if n_competing_components > 1:
                        if loadings_df_3_2[0] - loadings_df_3_2[1] >= delta:
                            v_PC_multi_list_3.append(
                                [
                                    var,
                                    n_competing_components,
                                    (
                                        loadings_df_3_2[
                                            loadings_df_3_2 == loadings_df_3_2[0]
                                        ].index
                                    )[0],
                                    None,
                                ]
                            ...
                            try:
                                v_PC_multi_list_3.append(
                                    [
                                        ...
                                        (
                                            loadings_df_3_2[
                                                loadings_df_3_2 == loadings_df_3_2[1]
                                            ].index
                                        )[0],
                                    ]
                                )
                            except:
                                v_PC_multi_list_3.append([var, 0, None, None])
            ...
            if show:
                print(
                    "The fist dataframe represents the distribution of variables across components."
                )
                display(PCs_through_vars)
                print("-------------")
                print(
                    "The second dataframe represents the distribution of components across variables."
                )
