...
from scipy.stats import chi2, chi2_contingency, kendalltau, norm, pearsonr, spearmanr
...
class Crosstab:
    ...
    def __init__(
        ...
        self._frequencies_observed = pd.crosstab(
        ...
        self.n_cells = (
            self._frequencies_observed.shape[0] * self._frequencies_observed.shape[1]
        )
        ...
        self.column_marginals = Crosstab._get_marginals(
            self._frequencies_observed, "column"
        )

        self.frequencies_observed = Crosstab._add_marginals(
            self._frequencies_observed,
            self.row_marginals,
        ...
        self.frequencies_expected = Crosstab._add_marginals(
            self._frequencies_expected,
            self.row_marginals,
            self.column_marginals,
            self.N,
        )
    ...
    def _get_marginals(crosstab, axis="row"):
        if axis == "row":
            marginals = pd.DataFrame(crosstab.sum(axis=1), columns=["Total"])
        elif axis == "column":
            marginals = pd.DataFrame(crosstab.sum(), columns=["Total"]).T
        else:
    ...
    def _get_expected_frequencies(self):
        frequencies_expected = np.array(
            [self.column_marginals.T["Total"].to_list()] * len(self.row_categories)
        )
        frequencies_expected = (
            frequencies_expected
            * self.row_marginals["Total"].to_numpy()[:, np.newaxis]
            / self.N
        )
        frequencies_expected = pd.DataFrame(
            frequencies_expected,
...
The smallest residual is {round(min_resid, n_decimals)} (categories {min_resid_row} and {min_resid_column}).""")
    ...
    def show_results(self, n_decimals=3):
        ...
        display(
            self.frequencies_observed.style.format(
        ...
        display(
            self.frequencies_expected.style.format(
                None, na_rep="", precision=n_decimals
        ...
        print("------------------\n")
        print(
            f"Chi-square statistic is {round(self.chi_square, n_decimals)} (p-value = {round(self.pvalue, n_decimals)})."
        )
        print("------------------\n")
        print("Pearson's residuals")
        display(
            self.residuals_pearson.style.format(
                None, na_rep="", precision=n_decimals
...
class Correlation:
    ...
    def __init__(
        ...
        if variables is not None and not isinstance(variables, list):
            raise TypeError(
                f"Variables should be passed as list. Type {type(variables)} was passed instead."
            )

        if not isinstance(method, str):
            raise TypeError(
                f"Method should be passed as str. Type {type(method)} was passed instead."
            )
        ...
        zero_var_vars = [
            ...
            if is_numeric_dtype(data[var]) and data[var].var() == 0
        ]
        variables = [
            var
            for var in variables
            if var not in non_num_vars and var not in zero_var_vars
        ]
        ...
        if len(variables) < 2:
            raise ValueError(
                "One or zero numeric variables were passed or found in a dataframe."
            )
        ...
        #             pass
            ...
            raise ValueError(
                "Unknown method requested. Possible values: 'pearson', 'spearman', 'kendall'"
            )
    ...
    def show_results(self, n_decimals=3, min_max=True):
        ...
        display(
            self.correlation_matrix.style.format(
                None, na_rep="", precision=n_decimals
        ...
        if min_max:
            ...
            print(
                f"Maximum correlation is {round(self.max_corr, n_decimals)} (p-value {max_pval}) for variables {idxmax[0]} and {idxmax[1]},",
