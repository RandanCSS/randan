...
def merge_two_intervals(series, cat1, cat2):
    ...
    return series.apply(
        lambda x: pd.Interval(lower_bound, upper_bound) if x in pair else x
    )
...
def classification_table(y_true, y_predicted):
    classification = pd.crosstab(y_true, y_predicted, margins=True)
    classification.index.name = "Observed"
    classification.columns.name = "Predicted"
    all_categories = list(classification.index)
    empty_categories = [
        value for value in all_categories if value not in classification.columns
    ]
    ...
    classification.loc["All", "All"] = (
        np.diagonal(classification.loc[all_categories, all_categories]).sum() / n * 100
    )
