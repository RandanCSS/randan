...
# .. метода getComments из API ВК, помогающая работе с ключами
def wallGetCommentsCore(API_keyS,
                        count,
                        fields,
                        iteration,
                        keyOrder,
    ...
    params = {'access_token': API_keyS[keyOrder], # обязательный параметр
              ...
              'need_likes': 1, # опциональный параметр
              'offset': offset, # опциональный параметр
              'owner_id': owner_id, # опциональный параметр, но без него не будет результата
              'post_id': post_id, # опциональный параметр, но без него не будет результата
    ...
    while goC:
        try: # чтобы обработать сигнал прерывания, поданный на любом этапе сбора данных
            ...
            if 'response' in response.keys():
                ...
                # print('    response.keys() внутри wallgetCommentsCore', response.keys()) # для отладки
            ...
            # print('goS wallgetCommentsCore:', goS) # для отладки
...
--- После ввода нажмите Enter'''
    ...
    if not silentMode: print('Количество ключей:', len(API_keyS), '\n')
...
--- Введите ID интересующего поста, после чего нажмите Enter'''
                ...
                if folderFile:
                    print(
'ВАЖНО! В результате исполнения его скрипта данные из указанного Вами файла', folderFile, 'будут дополнены актуальными данными из выдачи скрипта',
...
f'-- можете подать их в скобки функции wallGet перед её запуском или скопировать код исполняемого сейчас скрипта и сделать это внутри кода внутри метода {method} в разделе 2'
        ...
        while True:
            itemsAdditional, goS, iteration, keyOrder, pause, response = wallGetCommentsCore(API_keyS,
                                                                                             count,
                                                                                             fields,
                                                                                             iteration,
                                                                                             keyOrder,
