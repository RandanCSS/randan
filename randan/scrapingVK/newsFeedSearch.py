...
def bigSearch(
    ...
    params = {
              ...
              'count': 200, # опциональный параметр,
...
def newsFeedSearch(
    ...
    print('    Скрипт нацелен на выгрузку характеристик контента ВК методом его API newsfeed.search. Причём количество объектов выгрузки максимизируется путём её сегментирования по годам и месяцам.'
          , 'При этом следует учесть, что текущая версия API ВК, на которой и основан скрипт, "не умеет" обращаться к контенту, опубликованному ранее 2020 года'
...
# 2.0.4 Пользовательские настройки запроса к API ВК
        ...
        if (start_time == None) & (end_time == None) & (yearsRange == None): # если пользователь не подал эти аргументы в рамках experiencedMode
            while True:
                if folderFile != None: print('') # если пользователь не подал этот аргумент
                print('Если требуется конкретный временнОй диапазон, то можно использовать его не на текущем этапе выгрузки данных, а на следующем этапе -- предобработки датафрейма с выгруженными данными.'
                      ...
                      , '\n--- Кроме того, следует учесть, что текущая версия API ВК "не умеет" обращаться к контенту, опубликованному ранее 2020 года'
