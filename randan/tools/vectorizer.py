...
def vectProcess(frequencyMetric, column, df, min_df, ngram=1):
    ...
    vect = (
        CountVectorizer(min_df=min_df, ngram_range=(ngram, ngram)).fit(df[column])
        if frequencyMetric == "c"
        else TfidfVectorizer(min_df=min_df, ngram_range=(ngram, ngram)).fit(df[column])
    )
    matrix = vect.transform(df[column])  # оформить новый датафрейм..
    matrix_df = pandas.DataFrame(
        matrix.toarray(), columns=vect.get_feature_names_out(), index=df.index
    )  # .. у которого
...
def vectSettings(column, df, ngram=1):
    ...
    while goC_0:
        print(
            "--- Если хотите отсеять менее частотные токены, впишите предпочитаемую границу как дробное число -- минимальная доля встречаемости",
        ...
        print(
            "--- В результате векторизации можно получить абсолютные или относительные частоты токенов в документах (TF-IDF)",
            "\n--- Выше представлены наиболее и наименее характерные токены с т.з. абсолютных частот и TF-IDF",
        ...
        print(
            "--- Если результат Вас устраивает, нажмите Enter",
