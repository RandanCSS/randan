...
def df2file(dfIn, *arg):  # арки: fileName и folder
    ...
    if folder != "":
        print(
            f'Директория, в которую сохраняю файл "{fileName}":',
            os.getcwd() + slash + folder,
        )
        ...
        folder = input(
            "--- Впишите директорию, в которую сохранить файл (если имя файла уже содержит путь к нему, то не вписывайте ничего) и нажмите Enter:"
        )
    ...
    if (
        ...
        while True:
            print(
                "--- Если хотите сохранить датафрейм в файл Excel, нажмите Enter;",
        ...
        while True:
            try:
                ...
                if "IllegalCharacterError" in str(errorDescription[0]):
                    module = "xlsxwriter"
                    print(
                        "Для устранения ошибки требуется пакет",
                        {module},
                        "поэтому он будет инсталирован сейчас\n",
                    )
                    check_call([sys.executable, "-m", "pip", "install", module])
                    attempt += 1
                    if attempt == 10:
...
def df2fileShell(
    complicatedNamePart, dfIn, fileFormatChoice, method, coLabFolder, currentMoment
):
    slash = "\\" if os.name == "nt" else "/"  # выбор слэша в зависимости от ОС
    folder = currentMoment + complicatedNamePart
    if coLabFolder == None:
        print(
            "Сохраняю выгрузку метода", method, "                              "
        )  # , f'в директорию "{folder}"'
        ...
        print(
            "Сохраняю выгрузку метода", method, "                              "
        )  # , f'в директорию "{os.getcwd() + slash + coLabFolder + slash + folder}"'
    ...
    if len(columnsToJSON) > 0:
        print(
            "В выгрузке метода",
            method,
            "есть столбцы, содержащие внутри своих ячеек JSON-объекты; Excel не поддерживает JSON-формат;",
        ...
        if "from_id" in dfIn.columns:
            columnsToJSON.append("from_id")
        if "owner_id" in dfIn.columns:
            columnsToJSON.append("owner_id")

        df2file(
            dfIn[columnsToJSON],
            f"{folder}_{method}_JSON_varS.json",
            folder if coLabFolder == None else coLabFolder + slash + folder,
        )
        columnsToJSON.remove("id")
        if "from_id" in columnsToJSON:
            columnsToJSON.remove("from_id")
        if "owner_id" in columnsToJSON:
            columnsToJSON.remove("owner_id")

        df2file(
            dfIn.drop(columnsToJSON, axis=1),
            f"{folder}_{method}_Other_varS{fileFormatChoice}",
            folder if coLabFolder == None else coLabFolder + slash + folder,
        )
    else:
        df2file(
            dfIn,
            f"{folder}_{method}{fileFormatChoice}",
            folder if coLabFolder == None else coLabFolder + slash + folder,
        )
