...
def getFolder():
    folder = input(
        "--- С какой директорией хотите работать? Укажите полный путь к ней, включая её саму:"
    )
...
def getFolderFile(*arg):  # арки: fileName и folder
    ...
    if len(arg) == 0:
        # print(0)
        folderFile = input(
            "--- С каким файлом хотите работать? Укажите полный путь, включая название файла:"
        )
        folderFile = folderFile.replace(
            '"', ""
        )  # поскольку в Windows путь из Проводника закавычен
        # print(folderFile)
        fileName = folderFile.split(slash)[-1]
        # print(fileName)
        folder = slash.join(
            folderFile.split(slash)[:-1]
        )  # из полного пути убрать имя файла
        folder += slash
        # print(folder)
...
def files2df(*arg):
    ...
    # print(*arg)
    print(
        "Эта функция предназначена для оформления в датафрейм таблицы из файла формата Excel",
    ...
    if error == None:
        if len(folder) == 0:  # значит, из excel2df полный путь передан в fileName
            folder = (
                slash.join(fileName.split(slash)[:-1]) + slash
            )  # из полного пути убрать имя файла
            fileName = fileName.split(slash)[
                -1
            ]  # из полного пути оставить только имя файла
        ...
        for frmt in formatS:
            print(
                "\n--- Если требуется найти ещё файл формата",
                frmt,
                "для присоединения, то нажмите Enter",
                ...
                for fileName in fileNameS:
                    if ("." + frmt.lower()) in fileName:
                        print(
                            "--- Найден файл",
                            fileName,
                            ". Если он подходит, то нажмите Enter",
                        ...
                        if len(input()) == 0:
                            ...
                            print(
                                "--- Файл",
                                fileName,
                                "учтён; если требуется найти ещё файл формата",
                                frmt,
                                ", то нажмите Enter",
                                "\n--- Если НЕ требуется, то введите любой символ и нажмите Enter",
                            )
                ...
                if len(fileNamesForImport) > 0:
                    ...
                    try:
                        for fileName in fileNamesForImport:
                            ...
                            if "csv" in fileName:
                                df = df.merge(
                                    pandas.read_csv(f"{folder}{fileName}"),
                                    on="id",
                                    how="outer",
                                )
                            if "json" in fileName:
                                df = df.merge(
                                    pandas.read_json(f"{folder}{fileName}"),
                                    on="id",
                                    how="outer",
                                )
