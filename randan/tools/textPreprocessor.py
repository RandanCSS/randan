...
def autoCorrectorText(fast, language, text, tokensCorrectedQuantityMax, userWordS):
    ...
    for tokenPosition in range(len(tokenS)):
        ...
        # print('                                                                                ', end='\r')
        if token == tokenCorrected:
            tokensCorrectedActual.remove(tokenCorrected)
        else:
            ...
            if (
                tokenCorrected in spellerInstance.nlp_data.keys()
            ):  # как ни странно, автокоррект иногда берёт слова для исправления не из словаря своего (например, "Щёлкнул")
                if (
                    spellerInstance.nlp_data[tokenCorrected]
                    > tokensCorrectedQuantityMax
                ):
                    tokensCorrectedQuantityMax = spellerInstance.nlp_data[
                        tokenCorrected
                    ]
    correctionCoefficience = 100 * round(
        len(corrections.keys()) / len(numpy.unique(tokenS)), 4
    )
...
def autoCorrectorTextS(
    ...
    if probeMode:
        ...
        dfProbe["correctionResults"] = dfProbe[columnWithText].progress_apply(
            lambda text: autoCorrectorText(
                fast=False,
                language=language,
                text=text,
        ...
        correctionCoefficienceMean = (
            dfProbe["correctionResults"]
            .apply(lambda dictionary: dictionary["correctionCoefficience"])
            .mean()
        )
        ...
        print(
            f"Расчитанный на выборке текстов коэффициент исправлений равен {round(correctionCoefficienceMean, 2)}%"
        )
        if correctionCoefficienceMean < 5:
            print(
                "Поскольку он меньше 5% , применение автокорректора не рекомендуется или требует предварительного рассмотрения предлагаемых автокорректором исправлений"
            )
        else:
            print("Поскольку он НЕ меньше 5% , применение автокорректора рекомендуется")
        print(
...
Для ускорения можно в круглых скобках функции autoCorrectorTextS указать: fast=True"""
        ...
        if (
            ("correctionResults" in df.columns) & (userWordS == None)
        ):  # автокоррекция уже выполнялась пробно и пользователь НЕ запросил подстройку словаря, поэтому она будет применена только к остальным текстам
        ...
        dfIncorrected["correctionResults"] = dfIncorrected[
            columnWithText
        ].progress_apply(
            lambda text: autoCorrectorText(
                fast=False,
        ...
        correctionCoefficienceMean = (
            df["correctionResults"]
            .apply(lambda dictionary: dictionary["correctionCoefficience"])
            .mean()
        )
        print(
            f"Расчитанный на всех текстах коэффициент исправлений равен {round(correctionCoefficienceMean, 4)}%"
        )
    ...
    for row in (
        df["correctionResults"]
        .dropna()
        .apply(lambda dictionary: dictionary["corrections"])
    ):
        for key in row.keys():
    ...
    tokensCorrectedQuantityMax = (
        df["correctionResults"]
        .dropna()
        .apply(lambda dictionary: dictionary["tokensCorrectedQuantityMax"])
        .max()
    )

    print(
        "\nСловарь предлагаемых автокорректором исправлений (первые 100):",
        dict(itertools.islice(corrections.items(), 100)),
        "\ntokensCorrectedQuantityMax:",
        tokensCorrectedQuantityMax,
    )
...
def multispaceCleaner(text):
    ...
    if (
        len(text) > 0
    ):  # т.к., например, после чистки текста от эмодзи в нём может остаться пустота
        while "  " in textCleaned:
            textCleaned = textCleaned.replace("  ", " ")
        while textCleaned[0] == " ":
            textCleaned = textCleaned[1:]  # избавиться от пробелов в начале текста
        while textCleaned[-1] == " ":
            textCleaned = textCleaned[:-1]  # избавиться от пробелов в конце текста
    return textCleaned
...
def pymystemLemmatizer(dfIn, columnWithText):
    ...
    while len(df[df[columnWithText].str.contains(separator)]) == 0:
        print(
            "--- Не получается использовать разделитель",
            separator,
            "по которому тексты из всех ячеек сначала объединятся в один, а потом снова сепарируются",
    ...
    df[columnWithText] = df[
        columnWithText
    ].str.strip()  # убрать появившиеся после лемматизации \n на концах лемматизированных текстов
    df[columnWithText] = df[columnWithText].apply(
...
def simbolsCleaner(text):
    ...
    for a in text:
        if (a.isalnum()) | (a == " "):
            ...
            textCleaned += " "  # чтобы при удалении лишних символов, за которым не следует пробел, оставшиеся символы не сливались
    textCleaned = multispaceCleaner(textCleaned)
    return textCleaned


def stopwordsDropper(
    text, userStopWordsToAdd=None, userStopWordsToRemove=None, language="russian"
):
    """
    ...
    if userStopWordsToAdd != None:
        stopWordS.extend(userStopWordsToAdd)
    if userStopWordsToRemove != None:
        for word in userStopWordsToRemove:
