...
def jsonChecker(dfIn):
    ...
    for column in dfIn.columns:  # цикл для прохода по всем столбцам датафрейма
        ...
        if (
            dfIn[column]
            .apply(
                lambda cellContent: True
                if (type(cellContent) == dict) | (type(cellContent) == list)
                else False
            )
            .sum()
            == 0
        ):
            columnsToJSON.remove(
                column
            )  # .. то этот столбец исключается из "подозреваемых" методм .remove() класса списков
...
def valuesDropper(dfIn, var):
    df = dfIn.copy()
    print(
        "В этой функции переменная рассматривается как порядковая или более высокого типа шкалы"
    )
    varHist(df, var)
    print(
    ...
    if len(valueForRemoving) != 0:
        ...
        if len(valueForRemoving) != 0:
            ...
            if len(valueForRemoving) != 0:
                ...
                df = df[
                    (df[var] < float(valueS_ForRemoving[0]))
                    | (df[var] > float(valueS_ForRemoving[-1]))
                ]
