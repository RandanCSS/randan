    ...
    def fit(
        ...
        if variables is None:
            ...
            if not isinstance(variables, list):
                raise TypeError(
                    f"Variables should be passed as list. {type(variables)} was passed instead."
                )

            data = data[variables].dropna().copy()

        self._integer_type_vars = [
            var for var in variables if (data[var].apply(round) != data[var]).sum() == 0
        ]
...
To see more plots, use [model].get_bivariate_plots()""")
    ...
    def show_results(self, n_decimals=3):
        ...
        display(
            self.get_cluster_centers(round_discrete=True)
...
To see the exact centers, use [model].get_cluster_centers(round_discrete=False)""")
        ...
        display(
            self.get_distances_between_centers()
            .style.format(None, na_rep="", precision=n_decimals)
        ...
        display(
            self.get_number_of_cases_by_clusters()
            .style.format(None, na_rep="", precision=n_decimals)
    ...
    def get_cluster_centers(self, round_discrete=False):
        ...
        result = pd.DataFrame(centers, columns=variables, index=idx)
    ...
    def get_number_of_cases_by_clusters(self):
        ...
        dist = pd.DataFrame(preds["Cluster"].value_counts(dropna=False).sort_index())
        dist.columns = ["N"]
    ...
    def transform(
        ...
        cl_memb = pd.DataFrame(
            cl_memb, index=observation_idx, columns=["Cluster"], dtype=int
        )

        dist_cols = [f"Distance to cluster {i}" for i in self._cluster_idx]
        dist = pd.DataFrame(dist, index=observation_idx, columns=dist_cols)
    ...
    def _one_bivariate_cluster_plot(data, two_variables, centers):
        ...
        for i, c in enumerate(centers.values):
            k = i + 1
            plt.scatter(c[0], c[1], marker="$%d$" % k, alpha=1, s=50, edgecolor="k")
        plt.xlabel(var1)
        plt.ylabel(var2)
    ...
    def get_silhouette_plot(self):
        ...
        plt.barh(
            0.1, silh_point, left=-1, height=0.1, color="orchid", edgecolor="black"
        )
