...
class LinearRegression:
    ...
    def predict(
        ...
        if data is None:
            ...
            aux_data = pd.DataFrame(
                aux_model.model.exog, index=aux_data_idx, columns=aux_data_cols
            )
    ...
    def show_results(self, n_decimals):
        ...
        display(
            self.summary_r2()
            .style.set_caption(phrase.format(".summary_r2()"))
    ...
    def save_independent_variables(self, data=None, add_to_data=False):
        ...
        if data is None:
            ...
            result = pd.DataFrame(result, columns=columns, index=self._observations_idx)
    ...
    def save_residuals(
        ...
        columns_to_show = [
            f"{k.capitalize().replace('ized', '.').replace('eted', '.').replace('_', ' ')} res."
            for k, v in vars().items()
            if v == True and k != "add_to_data"
        ]
...
class BinaryLogisticRegression:
    ...
    def fit(
        ...
        if not is_numeric_dtype(self._data[self.dependent_variable]):
            self._data[self.dependent_variable] = (
                self._data[self.dependent_variable].map(self._mapper).astype(int)
            )
    ...
    def show_results(self, n_decimals):
        ...
        display(
            self.get_dependent_variable_codes().style.set_caption(
                phrase.format(".get_dependent_variable_codes()")
            )
        )
        print("------------------\n")
        print("Model summary")
        ...
        display(
            self.get_classification_table()
            .style.set_caption(phrase.format(".get_classification_table()"))
            .format(precision=n_decimals)
        )
        print("------------------\n")
        ...
        display(
            ...
            .style.set_caption(phrase.format(".get_precision_and_recall()"))
            .format(precision=n_decimals)
        )
        print("------------------\n")
        print("Coefficients")
    ...
    def get_classification_table(self):
        ...
        classification = pd.DataFrame(
            self._model.pred_table(), columns=self._dep_cats, index=self._dep_cats
        )
        ...
        for category in all_categories:
            classification.loc[category, "All"] = (
                classification.loc[category, category]
                / classification.loc[category, "All"]
                * 100
            )
            classification.loc["All", category] = (
                classification.loc["All", category] / n * 100
            )

        classification.loc["All", "All"] = (
            np.diagonal(classification.loc[all_categories, all_categories]).sum()
            / n
            * 100
        )
    ...
    def get_precision_and_recall(self):
        ...
        results = pd.DataFrame(
            results, index=categories, columns=["Precision", "Recall", "F score"]
        )
