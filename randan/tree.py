...
class CHAIDClassifier:
    ...
    def fit(
        ...
        if self._use_test_data:
            self._test_data = test_data.dropna(
        ...
        tree_nodes.extend(self._build_nodes_from_variable(data, dependent_variable))

        tree_nodes = self._split_nodes(
            data, dependent_variable, independent_variables, tree_nodes
        )

        self.nodes = pd.DataFrame(tree_nodes)

        if len(self.nodes) > 1:
            terminal_nodes_idx = self.nodes.apply(
                lambda x: self._check_if_terminal_node(x), axis=1
            )
            self.terminal_nodes = self.nodes[terminal_nodes_idx]
            terminal_nodes_lst = [tree_nodes[i] for i in self.terminal_nodes.index]
            depths = [
                CHAIDClassifier._get_node_depth(node, tree_nodes)
            ...
            self._data[f"{dependent_variable} (predicted)"] = (
                self._data[dependent_variable].mode().item()
            )
        ...
        if self._use_test_data:
            ...
            self.classification_table_test = classification_table(
                self._test_data[dependent_variable],
                test_predictions[f"{dependent_variable} (predicted)"],
            )
            self.precision_and_recall_test = precision_and_recall(
                self.classification_table_test
            )

        if show_results:
            self.show_results(
                tree_in_table_format=tree_in_table_format, n_decimals=n_decimals
            )
        if plot_tree:
            print("------------------\n")
            print("Tree plot")
    ...
    def plot_tree(
        ...
        for idx, node in self.nodes.iterrows():
            ...
            if pd.notnull(parent_node):
                graph.add_edge(
                    pydot.Edge(node_labels[parent_node], node_labels[current_node])
                )
        if save_plot:
            if save_plot_path is not None:
                path = os.path.join(
                    save_plot_path, f"{save_plot_name}.{save_plot_format}"
                )
            else:
            ...
            if save_plot_format.lower().strip() == "png":
                ...
                raise ValueError(
                    "Unknown image format. Available options: png, jpg, svg"
                )
    ...
    def show_results(self, tree_in_table_format=False, n_decimals=3):
        """
        ...
        if tree_in_table_format:
            ...
            print("Tree nodes")
            display(
                self.nodes.style.format(
                    None, na_rep="", precision=n_decimals
                ).set_caption(
                    f"Dependent variable: {self._dependent_variable}, independent variables: {', '.join(self._independent_variables)}"
                )
            )
        print("------------------\n")
        print("Classification table")
        display(self.classification_table.style.format(precision=1))
        if self._use_test_data:
            print("Classification table for test data")
            display(self.classification_table_test.style.format(precision=1))
        print("------------------\n")
        print("Prediction quality metrics")
        display(self.precision_and_recall.style.format(precision=n_decimals))
        if self._use_test_data:
            print("Prediction quality metrics for test data")
            display(self.precision_and_recall_test.style.format(precision=n_decimals))
    ...
    def get_classification_table(self):
        ...
        classification = classification_table(
            self._data[dependent_variable],
            self._data[f"{dependent_variable} (predicted)"],
        )
        return classification

    def _check_if_terminal_node(self, node):
        if node["Node"] in self.nodes["Parent node"].tolist() or pd.isna(
            node["Parent node"]
        ):
            return False
        else:
    ...
    @staticmethod
    def _predict_one_observation(observation, nodes, scale_variables, constant):
        ...
        while True:
            ...
            if split_variable not in scale_variables:
                try:
                    rule = (
                        current_nodes_level.loc[:, "Category"]
                        .str.split(" / ")
                        .apply(
                            lambda x: True if str(observation_category) in x else False
                        )
                    )
                    ...
                    rule = (
                        ...
                        .apply(
                            lambda x: True
                            if str(float(observation_category)) in x
                            else False
                        )
                    )
            else:
                rule = current_nodes_level.loc[:, "Category"].apply(
                    lambda x: x.overlaps(
                        pd.Interval(observation_category, observation_category, "both")
                    )
                )
    ...
    def predict(
        ...
        if data is None or data[self._independent_variables].equals(
            self._initial_data[self._independent_variables]
        ):
            result = self._data[
                [f"{self._dependent_variable} (predicted)", "Node"]
            ].copy()

        else:
            data_for_prediction = data[self.significant_variables].copy()
            result = data_for_prediction.apply(
                lambda x: CHAIDClassifier._predict_one_observation(
                    x, self.nodes, self._scale_variables, self._mode
                ),
                axis=1,
                result_type="expand",
            )
    ...
    @staticmethod
    def _get_most_significant_variable(
        data, dependent_variable, independent_variables, method
    ):
        ...
        if len(results) == 0:
            ...
            results = pd.DataFrame(
                results, columns=["pvalue", "chi2", "dof"], index=independent_variables
            )
    ...
    def _merging(self, data, dependent_variable, independent_variable):
        # todo - different methods in crosstab (pearson+likelihood)
        ...
        while max_pvalue > sig_level:
                ...
                if len(categories) == 2 and min_child_node is not None:
                    ...
                    if min_count < min_child_node:
                        if independent_variable not in self._scale_variables:
                            data[independent_variable] = merge_two_cats(
                                data[independent_variable], categories[0], categories[1]
                            )
                        else:
                            data[independent_variable] = merge_two_intervals(
                                data[independent_variable], categories[0], categories[1]
                            )
            ...
            if max_pvalue > sig_level:
                ...
                if independent_variable not in self._scale_variables:
                    data[independent_variable] = merge_two_cats(
                        data[independent_variable], cat1, cat2
                    )
                else:
                    data[independent_variable] = merge_two_intervals(
                        data[independent_variable], cat1, cat2
        ...
        if min_child_node is not None:
            ...
            while 0 < min_count < min_child_node and n_categories > 1:
                ...
                max_pvalue_category = pd.to_numeric(
                    results[min_count_category]
                ).idxmax()
                if independent_variable not in self._scale_variables:
                    data[independent_variable] = merge_two_cats(
    ...
    def _build_nodes_from_variable(
        self,
        data,
        dependent_variable,
        ...
        if independent_variable is not None:
            ...
            for category in categories:
                filter_data = data[data[independent_variable] == category][
                    dependent_variable
                ]
                n = len(filter_data)
            ...
            if len(small_n) < len(categories) - 1:
                for category in categories:
                    ...
                    n = len(filter_data)
                    dependent_variable_dist = dict(
                        (
                            round(
                                filter_data.value_counts(normalize=True, sort=False)
                                * 100,
                                3,
                            ).items()
                        )
            ...
            dependent_variable_dist = dict(
                (
                    round(
                        filter_data.value_counts(normalize=True, sort=False) * 100, 3
                    ).items()
                )
            )
            mode = filter_data.value_counts().idxmax()
    ...
    def _split_nodes(
        ...
        for variable in independent_variables:
            ...
            data_iter[variable] = self._merging(data_iter, dependent_variable, variable)

        split_variable, split_pvalue, split_chi2, split_dof = (
            CHAIDClassifier._get_most_significant_variable(
                data_iter, dependent_variable, independent_variables, self.method
            )
        )
        ...
        if split_variable is not None:
            ...
            if split_info["p-value"] <= self.sig_level_split:
                iter_nodes = self._build_nodes_from_variable(
                    data_iter,
                    dependent_variable,
                    split_variable,
                ...
                if len(iter_nodes) >= 2:
                    ...
                    if current_depth < self.max_depth:
                        for i in range(len(iter_nodes)):
                            ...
                            data_per_node = data[
                                data_iter[node_variable] == node_category
                            ].copy()
                            self._data.loc[
                                data_per_node.index,
                                f"{self._dependent_variable} (predicted)",
                            ] = node_prediction
    ...
    def get_interactions(self, result="dict"):
        ...
        results = self.nodes.apply(
            lambda x: CHAIDClassifier._get_one_node_interaction(x, self.nodes), axis=1
        )
        ...
        if result.lower() == "dict":
            ...
            raise ValueError(
                "Unknown result type. Possible values: 'dict' and 'DataFrame'."
            )
...
class CHAIDRegressor:
    ...
    def fit(
        ...
        if len(self.nodes) > 1:
            ...
            terminal_nodes_lst = [tree_nodes[i] for i in self.terminal_nodes.index]
            depths = [
                CHAIDRegressor._get_node_depth(node, tree_nodes)
                for node in terminal_nodes_lst
            ]
            self.depth = max(depths)
            ...
            self.depth = 0
            self.significant_variables = []
            self._data[f"{dependent_variable} (predicted)"] = self._data[
                dependent_variable
            ].mean()
    ...
    def plot_tree(
        ...
        for idx, node in self.nodes.iterrows():
            ...
            if current_node == "Node 0":
                node_0_ci = confidence_interval_mean(
                    predicted_category, std, n, sig_level=self.sig_level_split
                )
                ...
                node_x_ci = confidence_interval_mean(
                    predicted_category, std, n, sig_level=self.sig_level_split
                )
    ...
    def predict(
        ...
        if data is None:
            ...
            result = data_for_prediction.apply(
                lambda x: CHAIDRegressor._predict_one_observation(
                    x, self.nodes, self._scale_variables, self._mean
                ),
                axis=1,
                result_type="expand",
            )
    ...
    @staticmethod
    def _get_most_significant_variable(data, dependent_variable, independent_variables):
        results = []
        for variable in independent_variables:
            if len(data[variable].value_counts()) > 1:
    ...
    def _split_nodes(
        ...
        split_variable, split_pvalue, split_F, split_dof_b, split_dof_w = (
            CHAIDRegressor._get_most_significant_variable(
                data_iter, dependent_variable, independent_variables
            )
        )
    ...
    def get_interactions(self, result="dict"):
        ...
        results = self.nodes.apply(
            lambda x: CHAIDRegressor._get_one_node_interaction(x, self.nodes), axis=1
        )
        results.index = self.nodes["Node"]
        if result.lower() == "dict":
            return dict(results)
