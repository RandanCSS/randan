...
# Матрица "документы-топики" (и величины в ячейках матрицы названы так же)
    ...
    if (rowsNumerator != None) & (sum(df[rowsNumerator].isna()) == 0) & (len(list(set(df[rowsNumerator].tolist()))) == len(df)): df.index = df[rowsNumerator] # значения назначенного пользователем столбца будут использоваться в качестве обозначения строк итоговых таблиц, только если эти значения без NaN и без дубликатов
    elif len(list(set(list(df.index)))) != len(df):
        df.reset_index(drop=True) # в противном случае останутся исходные обозначения строк, но если эти значения без дубликатов -- иначе reset_index
        print(
'''
Назначенный Вами столбец для обозначения строк итоговых таблиц содержит NaN или дубликаты, поэтому не будет использоваться; также среди обозначений строк исходной таблицы есть дубликаты, поэтому эти обозначения заменяю на сквозную нумерацию.
'''
              )
    else: print(
'''
Назначенный Вами столбец для обозначения строк итоговых таблиц содержит NaN или дубликаты, поэтому будет использоваться не он, а обозначения строк исходной таблицы.
'''
                )
