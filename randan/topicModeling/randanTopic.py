...
def poleDocs_unique_search(df, docsLimit, pole, poleDocsIndeceS, textFull_lemmatized):
    poleDocS_df = df.loc[poleDocsIndeceS, :] # poleDocsDf
    ...
    if poleTokenS == []:
        ...
        for docIndex in poleDocsIndeceS:
            ...
            for token in poleTokenS:
                ...
                while goC:
                    try:
                        ...
                        # print('Поиск в документе', docIndex, 'по токену "', token, '"завершён безуспешно') # для отладки
                        goC = False
                # display('docSnippetS:', docSnippetS) # для отладки
            ...
            if len(docSnippetS) > 0:
                # Проход по всем токенам на обрабатываемом полюсе для объединения пересекающихся фрагментов из docSnippetS
                rowInvaderToDropS = []
...
# Добавить столбец со списками дублирующих (по столбцу textFull_lemmatized) каждый текст текстов и убрать неиспользуемые столбцы
    ...
    # if supplementarieS != None: scoreTextS = pandas.concat([scoreTextS, df[supplementarieS]], axis=1)
...
--- Если хотите получить фрагменты ключевых документов, относящихся к ключевым токенам, попробуйте снизить перечисленные пороги и перезапустить функцию randanTopic .'''
        ...
        message_tokens, tokenS_topic_minus_inUse, tokenS_topic_minus_inUse_list = tokensSelector(docS_minus, loadingsThreshold, -1, tokensLimit, topicLoadingS, topicName)
        message_tokens, tokenS_topic_plus_inUse, tokenS_topic_plnus_inUse_list = tokensSelector(docS_plus, loadingsThreshold, 1, tokensLimit, topicLoadingS, topicName)
        ...
        # display('topicLoadingS.loc[tokenS_topic_minus_inUse_list, topicName].mean():', topicLoadingS.loc[poleMinusTokenS, topicName].mean()) # для отладки
        ...
        minusDocs_snippetS, poleMinusDocsIndeceS = snippetByDoc(df, docsLimit, loadingsThreshold, poleS[0], poleMinusDocsIndeceS, poleMinusTokenS, supplementarieS, textFull_lemmatized, textFull_simbolsCleaned)
        # print('poleMinusDocsIndeceS:', poleMinusDocsIndeceS[:2]) # для отладки

        polePlusTokenS = list(topicTokenS[topicTokenS[topicName] > 0].sort_values(topicName, ascending=False).index)
        # print('polePlusTokenS:', polePlusTokenS) # для отладки
        # display('topicLoadingS.loc[polePlusTokenS, topicName].mean():', topicLoadingS.loc[polePlusTokenS, topicName].mean()) # для отладки
