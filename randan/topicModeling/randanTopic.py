...
def summaryPole(
    loadingsThreshold, minusPlus, tokensLimit, topicDocS, topicLoadingS, topicName
):
    summaryPole = pandas.DataFrame()
    if (
        len(
            topicLoadingS[topicLoadingS[topicName] * minusPlus > loadingsThreshold]
            .sort_values(topicName, ascending=False)
            .head(tokensLimit)
        )
        > 0
    ):
        ...
        summaryPole.loc[:, "Усреднённая связь токена с топиком"] = round(
            topicLoadingS[topicLoadingS[topicName] * minusPlus > loadingsThreshold]
            .sort_values(topicName, ascending=False)
            .head(tokensLimit)
            .mean()[topicName],
...
def snippetByDoc(
    df, loadingsThreshold, pole, poleDocsIndeceS, poleTokenS, supplementarieS
):
    print(f"{pole.capitalize()}ый полюс топика")
    if poleTokenS == []:
        print(
            f"Величина loadings токенов {pole}ого полюса не достигает заданного порога |{round(loadingsThreshold, 2)}|.",
            f"Поэтому {pole}ый полюс НЕ выражен и НЕ требует интерпретации",
        )
        docs_snippetS = pandas.DataFrame()
    else:
        print(
            f"{pole.capitalize()}ый полюс сформирован токен[ом, ами]",
            str(poleTokenS).replace("[", "").replace("]", ""),
            "-- по величине вклада",
        )
        print(
            f"На {pole}ом полюсе расположен[ы] документ[ы]",
            str(poleDocsIndeceS).replace("[", "").replace("]", ""),
            "-- по близости к полюсу",
        )
        docs_snippetS = pandas.DataFrame()
        for docIndex in poleDocsIndeceS:
            print(
                f"Посмотрите на фрагменты документа {docIndex}, содержащие указанные выше токены и их окружение.",
                "Документ:",
                docIndex,
            )
            row = 0
            ...
            for token in poleTokenS:
                ...
                while goC:
                    try:
                        ...
                        tokenPosition = textFull_lemmatized_list.index(
                            token, tokenPosition + 1
                        )

                        tokenPositionInterval = [
                            tokenPosition - tokenProximity,
                            tokenPosition + tokenProximity,
                        ]
            ...
            for tokenInvader in poleTokenS[:-1]:
                tokenInvaderIndex = docSnippetS[
                    docSnippetS["token"] == tokenInvader
                ].index
                ...
                for tokenReceiver in tokenReceiverS:
                    ...
                    tokenReceiverIndex = docSnippetS[
                        docSnippetS["token"] == tokenReceiver
                    ].index
                    for rowInvader in tokenInvaderIndex:
                        for rowReceiver in tokenReceiverIndex:
                            ...
                            if (
                                (
                                    docSnippetS["min"][rowInvader]
                                    >= docSnippetS["min"][rowReceiver]
                                )
                                & (
                                    docSnippetS["min"][rowInvader]
                                    <= docSnippetS["max"][rowReceiver]
                                )
                                | (
                                    docSnippetS["max"][rowInvader]
                                    >= docSnippetS["min"][rowReceiver]
                                )
                                & (
                                    docSnippetS["max"][rowInvader]
                                ...
                                docSnippetS.loc[rowReceiver, "min"] = min(
                                    docSnippetS["min"][rowInvader],
                                    docSnippetS["min"][rowReceiver],
                                )
                                docSnippetS.loc[rowReceiver, "max"] = max(
                                    docSnippetS["max"][rowInvader],
                                    docSnippetS["max"][rowReceiver],
                                )
                                docSnippetS.loc[rowReceiver, "token"] += (
                                    " " + tokenInvader
                                )
            ...
            for row in docSnippetS.index:
                ...
                docSnippetS.loc[row, "textSnippet"] = (
                    ".."
                    + " ".join(
                        textFull_list[docSnippetS["min"][row] : docSnippetS["max"][row]]
                    )
                    + ".."
                )
                if supplementarieS != None:
                    for supplementary in supplementarieS:
                        docSnippetS.loc[row, supplementary] = df[supplementary][
                            docIndex
                        ]
...
def randanTopic(
    ...
    matrix_df,
    docsLimit=5,
    loadingsThreshold=0.5,
    returnDfs=False,
    rowsNumerator=None,
    supplementarieS=None,
    textFull_lemmatized="textFull_lemmatized",
    textFull_simbolsCleaned="textFull_simbolsCleaned",
    tokensLimit=10,
    topicsCount=None,
):
...
--- После прочтения этой инструкции нажмите Enter"""
    ...
    if "textFull_lemmatized" not in dfIn.columns:
        print(
            """В поданном Вами датафрейме отсутствует столбец textFull_lemmatized . Какой столбец в поданном Вами датафрейме содержит лемматизированные тексты (из которого НЕ удалены стоп-слова)?"""
        )
        while True:
            print("--- Впишите, пожалуйста, его название")
            textFull_lemmatized = input()
            if textFull_lemmatized in dfIn.columns:
                break
            else:
    ...
    if "textFull_simbolsCleaned" not in dfIn.columns:
        print(
            """В поданном Вами датафрейме отсутствует столбец textFull_simbolsCleaned . Какой столбец в поданном Вами датафрейме содержит тексты, из которых удалены лишние символы, но НЕ лемматизированные?"""
        )
        while True:
            print("--- Впишите, пожалуйста, его название")
            textFull_lemmatized = input()
...
поскольку большее число топиков (а) вряд ли захотите проинтерпретировать и (б) расчёт ориентиров для них может занять слишком продолжительное время"""
        ...
        if topicsCount == None:
            ...
            встречаемостьТокенов = pandas.DataFrame(
                matrix_df.sum(), columns=["Встречаемость токенов"]
            )
            # display(встречаемостьТокенов)
            # Посчитать сумму по ДОКУМЕНТАМ
            токеныДокумент = pandas.DataFrame(
                matrix_df.T.sum(), columns=["Наполненность токенами документов"]
            )
            # display(токеныДокумент)
            # 1.1. Описательная статистика для интервальной переменной встречаемостьТокенов
            descriptive_statistics.ScaleStatistics(
                встречаемостьТокенов[["Встречаемость токенов"]]
            )
            # .. и для интервальной переменной токеныДокумент
            descriptive_statistics.ScaleStatistics(
                токеныДокумент[["Наполненность токенами документов"]]
            )

            # 2. Пробный запуск (без вращения, но с выводом результатов), чтобы посмотреть `Explained variance`
            dimension_reduction.PCA(
                n_components="Kaiser" if len(matrix_df.columns) < 30 else 30
            ).fit(matrix_df)
    ...
    if topicsCount > len(matrix_df.columns):
        print(
            "Число топиков принудительно снижено до",
            len(matrix_df.columns),
            ", поскольку значение topicsCount, равное",
            topicsCount,
            ", слишком велико для располагаемых данных.",
        )
    ...
    if rowsNumerator != None:
        if (sum(df[rowsNumerator].isna()) == 0) & (
            len(list(set(df[rowsNumerator].tolist()))) == len(df)
        ):
            df.index = df[
                rowsNumerator
            ]  # значения назначенного пользователем столбца будут использоваться в качестве обозначения строк итоговых таблиц, только если эти значения без NaN и без дубликатов
        else:
            print(
                """
...
"""
        ...
        if len(list(set(list(df.index)))) != len(df):
...
"""
    ...
    scoreTextS.index = df.index

    if supplementarieS != None:
        scoreTextS = pandas.concat([scoreTextS, df[supplementarieS]], axis=1)
    ...
    for topicName in topicNameS:
        ...
        if loadingsThreshold > maxLoading:
            print(
                "Порог (по модулю) усреднённой связи токена с топиком снижен до",
                round(maxLoading, 2),
                ", поскольку значение loadingsThreshold, равное",
                round(loadingsThreshold, 2),
                ", слишком велико для располагаемых данных.",
            )
            loadingsThreshold = (
                maxLoading * 0.99
            )  # не выше минимума из максимальных по модулю loadings среди компонент
        ...
        # Полярные документы
        topicDocS = pandas.concat(
            [
                scoreS.sort_values(topicName)[[topicName]].head(docsLimit),
                scoreS.sort_values(topicName)[[topicName]].tail(docsLimit),
            ]
        )
        # Полярные токены
        topicTokenS = pandas.concat(
            [
                topicLoadingS[topicLoadingS[topicName] < -loadingsThreshold]
                .sort_values(topicName)
                .head(tokensLimit),
                topicLoadingS[topicLoadingS[topicName] > loadingsThreshold]
                .sort_values(topicName)
                .tail(tokensLimit),
        ...
        summaryMinus = summaryPole(
            loadingsThreshold, -1, tokensLimit, topicDocS, topicLoadingS, topicName
        )
        summaryPlus = summaryPole(
            loadingsThreshold, 1, tokensLimit, topicDocS, topicLoadingS, topicName
        )
        ...
        poleMinusTokenS = list(
            topicTokenS[topicTokenS[topicName] < 0].sort_values(topicName).index
        )
        poleMinusDocsIndeceS = list(
            topicDocS[topicDocS[topicName] < 0].sort_values(topicName).index
        )
        minusDocs_snippetS = snippetByDoc(
            df,
            loadingsThreshold,
            poleS[0],
            poleMinusDocsIndeceS,
            poleMinusTokenS,
            supplementarieS,
        )
        ...
        plusDocs_snippetS = snippetByDoc(
            df,
            loadingsThreshold,
            poleS[-1],
            polePlusDocsIndeceS,
        ...
        docs_snippetS_additional = pandas.concat(
            [minusDocs_snippetS, plusDocs_snippetS]
        )
