...
# Импорт рейтинга с сайта moex.com
    ...
    # for issuer in issuerS_withoutRating[0:2]: # для отладки
        ...
        print("  ⏳ Ожидаю загрузки блока с рейтингами...")
        rating_header = WebDriverWait(driver, 10).until(
            expected_conditions.presence_of_element_located(
                (By.XPATH, "//h2[contains(., 'Кредитный рейтинг эмитента') or contains(., 'Кредитный рейтинг выпуска облигаций')]")
                                                            )
                                                        )
    
        # print(('Кредитный рейтинг эмитента\nРейтинг не присвоен' in driver.find_element("tag name", "body").text) &\
        #    ('Кредитный рейтинг выпуска облигаций\nРейтинг не присвоен' in driver.find_element("tag name", "body").text)) # для отладки
        if ('Кредитный рейтинг эмитента\nРейтинг не присвоен' in driver.find_element("tag name", "body").text) &\
           ('Кредитный рейтинг выпуска облигаций\nРейтинг не присвоен' in driver.find_element("tag name", "body").text):
            print('  ❌ Рейтинг не присвоен') # для отладки        
        else:
    ...
    # УНИВЕРСАЛЬНЫЙ ПАРСИНГ ТАБЛИЦЫ (работает с любым количеством столбцов)
            ...
            if not headers:
                ...
                print("    ⚠️ Заголовки таблицы не найдены, использую стандартные")
            else:
                print(f"  Обнаружены столбцы: {headers}")
    ...
    print('Остались ли облигации без рейтинга?')
