...
class NominalStatistics:
    ...
    def __init__(
        self, data, variables=None, frequencies=True, show_results=True, n_decimals=3
    ):
        if variables is not None:
            if not isinstance(variables, list):
                phrase = (
                    "Variables should be passed as list. Type {} was passed instead."
                )
    ...
    def _get_statistics(self):
        ...
        measures = pd.DataFrame(
            measures, index=["N", "mode", "entropy coef.", "quality var."]
        )
        return measures.T
...
class OrdinalStatistics:
    ...
    def __init__(
        ...
        if len(self._mappers) > 0:
            for var in self._mappers.keys():
                self._data[var] = (
                    self._data[var].map(self._mappers[var][0]).astype(float)
                )
    ...
    def _get_mappers_for_nonumerical_vars(self):
        nonnum_vars = [
            var for var in self._variables if not is_numeric_dtype(self._data[var])
        ]
        mappers = {}
        for var in nonnum_vars:
            mappers.update(
                {var: OrdinalStatistics._get_mappers_for_one_var(self._data[var])}
            )
        return mappers
        ...
        if len(self._mappers) > 0:
            for var in self._mappers.keys():
                measures.loc[["mode", "median", "25%", "75%", "min", "max"], var] = (
                    measures.loc[
                        ["mode", "median", "25%", "75%", "min", "max"], var
                    ].map(self._mappers[var][1])
                )
        return measures.T
...
class ScaleStatistics:
        ...
        if show_results:
            if len(self._mappers) > 0:
                ...
                for var in self._mappers:
                    ...
                    print(f"variable: {var}")
                    display(
                        pd.DataFrame(self._mappers[var][0], index=["Encoded value"]).T
                    )
    ...
    def show_normality_test(self, n_decimals=3):
        ...
        print("------------------\n")
        display(
            self.normality_test(self.normality_test_type)
            .style.format(None, na_rep="", precision=n_decimals)
            .set_caption(
                f"method .normality_test(test_type='{self.normality_test_type}')"
            )
        )
    ...
    def _get_mappers_for_nonumerical_vars(self):
        ...
        for var in nonnum_vars:
            mappers.update(
                {var: ScaleStatistics._get_mappers_for_one_var(self._data[var])}
            )
        return mappers
