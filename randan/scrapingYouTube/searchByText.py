...
while True:
    try:
        ...
        if  attempt == 10:
            print(
'Пакет', module, 'НЕ прединсталлируется с установкой Анаконды, для работы скрипта требуется этот пакет,',
...
# 2.0.3 Если такие данные, сохранённые при прошлом запуске скрипта, не найдены, возможно, пользователь хочет подать свои данные для их дополнения
    if temporalName == None: # если itemsTemporal, в т.ч. пустой, не существует
        ...
        print('--- Возможно, Вы располагаете файлом, в котором есть, как минимум, столбец id, и который хотели бы дополнить? Или планируете первичный сбор контента?'
...
# ----------
        ...
        if q == None: # если пользователь не подал этот аргумент в рамках experiencedMode
            print(
'''Скрипт умеет искать контент по текстовому запросу-фильтру
При этом если требуется контент конкретного канала, то лучше использовать запросу-фильтр НЕ на текущем этапе выгрузки данных,
