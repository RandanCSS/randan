...
while True:
    try:
        ...
        from randan.tools import (
            ...
            files2df,
            varPreprocessor,
        )  # авторский модуль для работы с календарём конкретного года  # авторский модуль для адаптации текущего скрипта к файловой системе CoLab  # авторский модуль для сохранения датафрейма в файл одного из форматов: CSV, Excel и JSON в рамках работы с данными из социальных медиа  # авторский модуль для оформления в датафрейм таблиц из файлов формата CSV, Excel и JSON в рамках работы с данными из социальных медиа  # авторский модуль для предобработки переменных номинального, порядкового, интервального и более высокого типа шкалы
...
def bigSearch(
    ...
    return (
        ...
        keyOrder,
        response,
    )  # от response отказаться нельзя, т.к. в нём много важных ключей, даже если их значения нули


# 1.1 для обработки выдачи метода channels, помогающая работе с ключами
def channelProcessor(
    API_keyS,
    channelIdForSearch,
    coLabFolder,
    complicatedNamePart,
    ...
    if len(df) > 0:  # если использовался search и успешно, id каналов берутся из него
        ...
        if len(channelIdS) > 0:
            channelIdS = (
                channelIdS[f"id.{snippetContentType}Id"].to_list()
                if f"id.{snippetContentType}Id" in channelIdS.columns
                else channelIdS["id"].to_list()
            )
    else:
        channelIdS = [
            channelIdForSearch
        ]  # в другом случае id канала подаётся пользователем
    # channelIdS = channelIdS[:5] # для отладки
    # print(channelIdS)

    method = "channels"
    print(
        "В скрипте используются следующие аргументы метода",
        method,
        'API YouTube: part=["snippet", "brandingSettings", "contentDetails", "id", "localizations", "statistics", "status", "topicDetails"], id, maxResults .',
        "Эти аргументы, кроме part, пользователю скрипта лучше не кастомизировать во избежание поломки скрипта.",
        "Если хотите добавить другие аргументы метода",
        method,
        "API YouTube, можете ознакомиться с ними по ссылке: https://developers.google.com/youtube/v3/docs/channels",
    )
...
--- Если НЕ хотите пополнить список, нажмите пробел и затем Enter"""
        ...
        if len(input()) == 0:
            if len(playlistS) > 0:
                ...
                playlistChannelId_list = (
                    playlistS["snippet.videoOwnerChannelId"].str.split(", ").to_list()
                )
    ...
    if len(channelIdS) > 0:
        print(
            f"""Проход по каналам{" порциями по 50 штук" if 90 > 50 else ""} для выгрузки их характеристик (дополнительных к выруженным методом search)"""
        )
        ...
        df2file.df2fileShell(
            ...
            dfIn=channelS,
            fileFormatChoice=fileFormatChoice,
            method=method.split(".")[0] + method.split(".")[1].capitalize()
            if "." in method
            else method,  # чтобы избавиться от лишней точки в имени файла
            coLabFolder=coLabFolder,
            currentMoment=momentCurrent.strftime(
                "%Y%m%d_%H%M"
...
def dfsProcessor(
    ...
    if (
        columnsForCheck == []
    ):  # для выдач, НЕ содержащих столбец id, проверка дублирующихся строк возможна по столбцам, содержащим в имени id
        for column in df.columns:
            if "id." in column:
                columnsForCheck.append(column)
    # print('Столбцы, по которым проверяю дублирующиеся строки:', columnsForCheck) # для отладки
    df = df.drop_duplicates(
        columnsForCheck, keep="last"
    ).reset_index(
        drop=True
    ...
    if goS == False:
        ...
        if not os.path.exists(
            f"{momentCurrent.strftime('%Y%m%d')}{complicatedNamePart}_Temporal"
        ):
            os.makedirs(
                f"{momentCurrent.strftime('%Y%m%d')}{complicatedNamePart}_Temporal"
            )
            print(
                f'Директория "{momentCurrent.strftime("%Y%m%d")}{complicatedNamePart}_Temporal" создана'
            )
        # else:
        # print(f'Директория "{momentCurrent.strftime("%Y%m%d")}{complicatedNamePart}_Temporal" существует') # для отладки

        file = open(
            f"{momentCurrent.strftime('%Y%m%d')}{complicatedNamePart}_Temporal{slash}channelIdForSearch.txt",
            "w+",
        )  # открыть на запись
        ...
        file = open(
            f"{momentCurrent.strftime('%Y%m%d')}{complicatedNamePart}_Temporal{slash}contentType.txt",
            "w+",
        )  # открыть на запись
        ...
        file = open(
            f"{momentCurrent.strftime('%Y%m%d')}{complicatedNamePart}_Temporal{slash}method.txt",
            "w+",
        )  # открыть на запись
        file.write(method)
        file.close()
        ...
        file = open(
            f"{momentCurrent.strftime('%Y%m%d')}{complicatedNamePart}_Temporal{slash}q.txt",
            "w+",
        )  # открыть на запись
        ...
        file = open(
            f"{momentCurrent.strftime('%Y%m%d')}{complicatedNamePart}_Temporal{slash}stageTarget.txt",
            "w+",
        )
        ...
        file = open(
            f"{momentCurrent.strftime('%Y%m%d')}{complicatedNamePart}_Temporal{slash}targetCount.txt",
            "w+",
        )
        ...
        file = open(
            f"{momentCurrent.strftime('%Y%m%d')}{complicatedNamePart}_Temporal{slash}year.txt",
            "w+",
        )
        ...
        file = open(
            f"{momentCurrent.strftime('%Y%m%d')}{complicatedNamePart}_Temporal{slash}yearsRange.txt",
            "w+",
        )
        file.write(
            yearsRange if yearsRange != None else ""
...
def downloadComments(API_keyS, sourceId, keyOrder, method):
    goS = True
    ...
    while True:  # прерывается командой break при отсутствии nextPageToken
        ...
        while goC:  # цикл позволяет возвращяться со следующим keyOrder к прежнему id при истечении квоты текущего ключа
            try:
                ...
                print(
                    "\nОшибка внутри авторской функции downloadComments"
                )  # для отладки
                goC, goS, keyOrder, problemItemId = errorProcessor(
                    errorDescription=sys.exc_info(),
                    keyOrder=keyOrder,
...
def errorProcessor(errorDescription, keyOrder, sourceId):
    ...
    if ("exceeded" in str(errorDescription[1]).lower()) & (
        "quota" in str(errorDescription[1]).lower()
    ):
        print(
    ...
    elif ("desabled" in str(errorDescription[1]).lower()) & (
        "key" in str(errorDescription[1]).lower()
    ):
        print(
            "!!! Похоже, текущий ключ деактивирован владельцем или Гуглом; пробую перейти к следующему ключу"
        )
        # print('  keyOrder ДО смены ключа', keyOrder) # для отладки
        keyOrder += 1  # смена ключа
        # print('  keyOrder ПОСЛЕ смены ключа', keyOrder) # для отладки
        ...
        print(
            "!!! Похоже, ключи закончились. Что делать? (а) Подождите сутки для восстановления ключей или",
        ...
        ("channel" in str(errorDescription[1]).lower())
        | ("comment" in str(errorDescription[1]).lower())
        | ("playlist" in str(errorDescription[1]).lower())
        | ("video" in str(errorDescription[1]).lower())
    ):
        print(
            "  Проблема может быть связана с ограничением доступа к обрабатываемому объекту, поэтому фиксирую его id:",
            problemItemId,
        )
        # print('problemItemId:', problemItemId) # для отладки
    elif "TimeoutError" in str(errorDescription[0]):
        print(
            "!!! Похоже, проблема в слишком высокой частоте запросов к удалённому серверу; засыпаю на 10 миллисекунд"
        )
        time.sleep(10)
    else:
        print(
            "!!! Похоже, проблема не в ограничении доступа к обрабатываемому объекту и не в истечении квоты текущего ключа(("
        )
...
def iterationVisualization(idS, iteration, portion, response):
    if idS != None:
        iterationUpperBound = (
            int(str(len(idS) / portion).split(".")[0]) + 1
        )  # дробная часть после деления числа idS должна увеличить iterationUpperBound на единицу
    print(
        f"""  Порция № {iteration + 1}{f" из {iterationUpperBound}" if idS != None else ""}.{f" Сколько в порции наблюдений? {len(response['items'])}" if portion > 1 else ""}""",
        end="\r",
    )


# 1.6 для обработки выдачи методов playlists и playlistItems, помогающая работе с ключами
def playListProcessor(
    API_keyS,
    channelIdForSearch,
    coLabFolder,
    complicatedNamePart,
    ...
    print(
        "В скрипте используются следующие аргументы метода",
        method,
        'API YouTube: part=["snippet", "contentDetails", "localizations", "status"], id, maxResults .',
        "Эти аргументы, кроме part, пользователю скрипта лучше не кастомизировать во избежание поломки скрипта.",
        "Если хотите добавить другие аргументы метода",
        method,
        "API YouTube, можете ознакомиться с ними по ссылке:",
    ...
    if len(playlistIdS) > 0:
        print(
            f"""Проход по плейлистам{" порциями по 50 штук" if len(playlistIdS) > 50 else ""} для выгрузки их характеристик (дополнительных к выруженным методом search)"""
        )
        ...
        print(
            "В скрипте используются следующие аргументы метода",
            method,
            'API YouTube: part=["snippet"], playlistId, maxResults .',
            "Эти аргументы, кроме part, пользователю скрипта лучше не кастомизировать во избежание поломки скрипта.",
            "Если хотите добавить другие аргументы метода",
            method,
            "API YouTube, можете ознакомиться с ними по ссылке:",
        ...
        playlistVideoChannelS = (
            pandas.DataFrame()
        )  # хотя датафреймы и глобальны как переменные, пусть и тут инициализируется
        portion = 50
        print(
        ...
        for playlistId in playlistIdS:
            ...
            while True:
                ...
                while (
                    goC
                ):  # цикл на случай истечения ключа: повторяет запрос после смены ключа
                    try:
                        youtube = api.build(
                            "youtube", "v3", developerKey=API_keyS[keyOrder]
                        )
                        response = (
                        ...
                        addPlaylistVideoChannelS = pandas.json_normalize(
                            response["items"]
                        )
                ...
                iterationVisualization(
                    idS=None, iteration=iteration, portion=portion, response=response
        ...
        print(
            "                                                                                                              "
        )  # затираю последнюю визуализацию
        ...
        for playlistId in playlistIdS:
            for column in ["snippet.resourceId.videoId", "snippet.videoOwnerChannelId"]:
                playlistVideoChannelS_snippet = playlistVideoChannelS[
                    playlistVideoChannelS[column].notna()
                ]
                playlistS.loc[
                    playlistS[playlistS["id"] == playlistId].index[0], column
                ] = ", ".join(
                    playlistVideoChannelS_snippet[
                        playlistVideoChannelS_snippet["snippet.playlistId"]
                        == playlistId
                    ][column].to_list()
                )
        # display(playlistS)
    ...
    while bound < len(idS):
        ...
        while goC:  # цикл на случай истечения ключа: повторяет запрос после смены ключа
            try:
                ...
                print(
                    "\nОшибка внутри авторской функции portionsProcessor"
                )  # для отладки
                goC, goS, keyOrder, problemItemId = errorProcessor(
                    errorDescription=sys.exc_info(), keyOrder=keyOrder, sourceId=None
                )
        # print('len(idS):', len(idS)) # для отладки
        iterationVisualization(
            idS, iteration, portion, response
        )  # для визуализации процесса через итерации
        iteration += 1
        bound += portion
...
def searchByText(
    ...
    channelS = (
        pandas.DataFrame()
    )  # чтобы обращаться к контейнеру, даже если функция, создающая его, не исполнялась
    ...
    stageTarget = (
        0  # stageTarget принимает значения [0; 3] и относится к стадиям скрипта
    )
    targetCount = 0
    temporalName = None
    videoS = (
    ...
    print(
        ...
        "-- он будет использована для формирования имён создаваемых директорий и файлов",
    )
    year = int(
        momentCurrent.strftime("%Y")
    )  # в случае отсутствия пользовательского временнОго диапазона
    ...
    if access_token == None:
        print(
            "Проверяю наличие файла credentialsYouTube.txt с ключ[ом ами], гипотетически сохранённым[и] при первом запуске скрипта"
        )
        if "credentialsYouTube.txt" in rootNameS:
            file = open(
                "credentialsYouTube.txt"
                if coLabFolder == None
                else coLabFolder + slash + "credentialsYouTube.txt"
            )
            API_keyS = file.read()
            print(
                "Нашёл файл credentialsYouTube.txt; далее буду использовать ключ[и] из него:",
                API_keyS,
            )
        else:
            print(
...
--- После ввода нажмите Enter"""
            ...
            while True:
                ...
                if len(API_keyS) != 0:
                    ...
                    from randan.tools.textPreprocessor import (
                        multispaceCleaner,
                    )  # авторский модуль для предобработки нестандартизированного текста

                    API_keyS = multispaceCleaner(API_keyS)
                    while API_keyS[-1] == ",":
                        API_keyS = API_keyS[:-1]  # избавиться от запятых в конце текста
    ...
    for rootName in rootNameS:
        if "Temporal" in rootName:
            if len(os.listdir(rootName)) == 9:
                ...
                file.close()
                if contentTypeTemporal == "":
                    contentTypeTemporal = None  # для единообразия

                file = open(f"{rootName}{slash}channelIdForSearch.txt")
                channelIdForSearchTemporal = file.read()
                file.close()
                if channelIdForSearchTemporal == "":
                    channelIdForSearchTemporal = None  # для единообразия
                ...
                file.close()
                if qTemporal == "":
                    qTemporal = None  # для единообразия

                file = open(f"{rootName}{slash}yearsRange.txt")
                yearsRangeTemporal = file.read()
                file.close()
                if yearsRangeTemporal == "":
                    yearsRangeTemporal = None  # для единообразия
                ...
                print(
                    ...
                    "\n- было выявлено целевое число записей (totalResults)",
                    targetCountTemporal,
                    "\n- скрипт остановился на методе",
                    methodTemporal,
                )
                if yearTemporal != None:
                    print("- и на годе (при сегментировани по годам)", yearTemporal)
                print(
                    "- пользователь НЕ определил тип контента"
                    if contentTypeTemporal == None
                    else f'- пользователь определил тип контента как "{contentTypeTemporal}"'
                )
                if contentTypeTemporal == "video":
                    print(
                        "- пользователь НЕ выбрал конкретный канал для выгрузки видео"
                        if channelIdForSearchTemporal == None
                        else f'- пользователь выбрал канал с id "{channelIdForSearchTemporal}" для выгрузки видео'
                    )
                print(
                    "- пользователь НЕ сформулировал запрос-фильтр"
                    if qTemporal == None
                    else f'- пользователь сформулировал запрос-фильтр как "{qTemporal}"'
                )
                print(
                    "- пользователь НЕ ограничил временнОй диапазон"
                    if yearsRangeTemporal == ""
                    else f"- пользователь ограничил временнОй диапазон границами {yearsRangeTemporal}"
                )
                print(
...
--- Если хотите найти другие промежуточные результаты, нажмите пробел и затем Enter"""
                ...
                if len(decision) == 0:  # Temporal-значения обретают статус постоянных
                    ...
                    for temporalName in temporalNameS:
                        if ".xlsx" in temporalName:
                            break
                    itemS = pandas.read_excel(
                        f"{rootName}{slash}{temporalName}", index_col=0
                    )

                    for temporalName in temporalNameS:
                        if ".json" in temporalName:
                            itemS = itemS.merge(
                                pandas.read_json(f"{rootName}{slash}{temporalName}"),
                                on="id",
                                how="outer",
                            )
                            break
                    ...
                    if yearsRange != None:
                        ...
                        yearMaxByUser, yearMinByUser, yearsRange = (
                            calendarWithinYear.yearsRangeParser(yearsRange)
                        )
    ...
    if temporalName == None:  # если itemsTemporal, в т.ч. пустой, не существует
        ...
        rootName = "No folder"
        print(
            "Не найдены подходящие данные, гипотетически сохранённые при прошлом запуске скрипта"
        )
        print(
            """
...
--- Если располагаете файлом, укажите полный путь, включая название файла, и нажмите Enter"""
            ...
            if len(folderFile) == 0:
                ...
                if error != None:
                    if "No such file or directory" in error:
                        print(
                            "Файл:",
                            folder + slash + fileName,
                            "-- не существует; попробуйте, пожалуйста, ещё раз..",
                        )
                else:
                    break
            # display(itemS)
        ...
        if (
            contentType == None
        ):  # если пользователь не подал этот аргумент в рамках experiencedMode
            while True:
                print(
                    "--- Если НЕ требуется определить тип искомого контента, нажмите Enter",
                ...
                if contentType.lower() == "":
                    ...
                    print(
                        "--- Вы ввели что-то не то; попробуйте, пожалуйста, ещё раз.."
                    )

        if (
            (channelIdForSearch == None) & (contentType == "video")
        ):  # если пользователь не подал аргумент channelIdForSearch в рамках experiencedMode
            print(
...
--- Если предполагается такой поиск, введите id канала, после чего нажмите Enter. Этот id можете найти либо в URL-адресе интересующего канала, либо -- если прежде выгружали контент из YouTube -- в столбце "snippet.channelId" выдачи методов search, playlistItems, videos или в столбце "id" метода cannels"""
            ...
            while True:
                ...
                # ----------
                    ...
                    while channelIdForSearch[-1] == ",":
                        channelIdForSearch = channelIdForSearch[
                            :-1
                        ]  # избавиться от запятых в конце текста
...
(описание, название, теги, категории и т.п.) релевантного YouTube-контента, после чего нажмите Enter"""
            ...
            if folderFile != None:
                print(
                    "ВАЖНО! В результате исполнения текущего скрипта данные из указанного Вами файла",
                    folderFile,
            ...
            q = input()
            if q == "":
                q = None  # для единообразия
            else:
                print("")

        # Ограничения временнОго диапазона
        if (
            (publishedAfter == None) & (publishedBefore == None) & (yearsRange == None)
        ):  # если пользователь не подал эти аргументы в рамках experiencedMode
            print(
...
--- Если всё же требуется задать временнОй диапазон, настоятельная рекомендация задать его годами, а не более мелкими единицами времени. Для задания диапазона введите без кавычек минимальный год диапазона, тире, максимальный год диапазона (минимум и максимум могут совпадать в такой записи: "год-тот же год") и нажмите Enter"""
                    ...
                    if "-" in yearsRange:
                        ...
                        if len(yearsRange) == 2:
                            if (len(yearsRange[0]) == 4) & (len(yearsRange[1]) == 4):
                                ...
                                print(
                                    "--- Вы ввели год[ы] НЕ из четырёх цифр. Попробуйте ещё раз.."
                                )
                        else:
                            print(
                                "--- Вы ввели тире, но при этом ввели НЕ два года. Попробуйте ещё раз.."
                            )
                    else:
        ...
        if publishedAfter != None:
            yearMinByUser = int(
                datetime.strptime(publishedAfter, "%Y-%m-%dT%H:%M:%SZ").strftime("%Y")
            )  # из experiencedMode
            # print('elif start_time != None:', yearMinByUser) # для отладки

        if publishedBefore != None:
            yearMaxByUser = int(
                datetime.strptime(publishedBefore, "%Y-%m-%dT%H:%M:%SZ").strftime("%Y")
            )  # из experiencedMode
            # print('elif end_time != None:', yearMaxByUser) # для отладки
            year = yearMaxByUser

        if (yearMinByUser != None) & (yearMaxByUser == None):
            yearMaxByUser = int(
                momentCurrent.strftime("%Y")
            )  # в случае отсутствия пользовательской верхней временнОй границы при наличии нижней
        elif (yearMinByUser == None) & (yearMaxByUser != None):
            yearMaxByUser = 1970  # в случае отсутствия пользовательской нижней временнОй границы при наличии верхней

        # print('yearMinByUser', yearMinByUser) # для отладки
        # print('yearMaxByUser', yearMaxByUser) # для отладки

        if (publishedAfter == None) & (yearMinByUser != None):
            publishedAfter = datetime(yearMinByUser, 1, 1).isoformat() + "Z"
        if (publishedBefore == None) & (yearMaxByUser != None):
            publishedBefore = datetime(yearMaxByUser, 12, 31).isoformat() + "Z"

        if yearsRange != None:
            print("")  # чтобы был отступ, если пользователь подал этот аргумент
    ...
    complicatedNamePart += (
        "" if channelIdForSearch == None else "_channelId_" + channelIdForSearch
    )
    if q != None:
        complicatedNamePart += "_" + q if len(q) < 50 else "_" + q[:50]
    complicatedNamePart += (
        ""
        if ((yearMinByUser == None) & (yearMaxByUser == None))
        else "_" + str(yearMinByUser) + "-" + str(yearMaxByUser)
    )
    ...
    if (
        (channelIdForSearch == None) | (q != None) | (yearsRange != None)
    ):  # в противном случае search следует заменить на cannels + playlistItems
        stage = 0
        iteration = 0  # номер итерации применения текущего метода
        method = "search"
        print(
        ...
        if (
            stage >= stageTarget
        ):  # eсли нет временного файла stage.txt с указанием пропустить этап
            ...
            print(
                "  Искомых объектов",
                targetCount,
                ", а найденных БЕЗ включения каких-либо значений аргумента order:",
                len(itemS),
            )
        ...
        if (
            stage >= stageTarget
        ):  # eсли НЕТ файла с id и нет временного файла stage.txt с указанием пропустить этап
            if len(itemS) < targetCount:
                ...
                print(
                    "Проход по значениям аргумента order, внутри которых проход по всем страницам выдачи"
                )
                for order in orderS:
                    addItemS, goS, iteration, keyOrder, response = bigSearch(
                        API_keyS=API_keyS,
                    ...
                    print(
                        "  Проход по всем следующим страницам с выдачей с тем же значением аргумента order:",
                        order,
                        "          ",
                    )
                    while (
                        ("nextPageToken" in response.keys())
                        & (len(itemS) < targetCount)
                ...
                print(
                    "  Искомых объектов",
                    targetCount,
                    ", а найденных С включением аргумента order:",
                    len(itemS),
                )
            else:
                print(
                    "Все искомые объекты найдены БЕЗ включения некоторых значений аргумента order (в т.ч. вообще БЕЗ них)"
                )
        elif stage < stageTarget:
            print(
                f'\nЭтап {stage} пропускаю согласно настройкам из файла stage.txt в директории "{rootName}"'
        ...
        if (
            ...
            if len(itemS) < targetCount:
                ...
                if len(input()) == 0:
                    print(
                        "Внутри каждого года прохожу по значениям аргумента order, внутри которых прохожу по всем страницам выдачи"
                    )
                    goC = True
                    # ********** из фрагмента 2.1.0 + условие для goC
                    while (len(itemS) < targetCount) & (goC):
                        print(
                            f"  Для года {year} заход на первую страницу выдачи БЕЗ аргумента order"
                        )
                        addItemS, goS, iteration, keyOrder, response = bigSearch(
                            API_keyS=API_keyS,
                            channelIdForSearch=channelIdForSearch,
                        ...
                        print(
                            f"    Проход по всем следующим страницам с выдачей для года {year} БЕЗ аргумента order"
                        )
                        while "nextPageToken" in response.keys():
                            pageToken = response["nextPageToken"]
                            addItemS, goS, iteration, keyOrder, response = bigSearch(
                        ...
                        print(
                            f"    Искомых объектов в году {year}: {targetCount}, а найденных БЕЗ включения каких-либо значений аргумента order:",
                            len(itemS),
                        )
                        # ********** из фрагмента 2.1.1
                        if len(itemS) < targetCount:
                            print(
                                f"  Для года {year} проход по значениям аргумента order,",
                                "внутри которых проход по всем страницам выдачи",
                            )
                            for order in orderS:
                                addItemS, goS, iteration, keyOrder, response = (
                                    bigSearch(
                                        API_keyS=API_keyS,
                                        channelIdForSearch=channelIdForSearch,
                                        channelType=channelType,
                                ...
                                print(
                                    f"    Для года {year} проход по всем следующим страницам с выдачей с тем же значением аргумента order:",
                                    order,
                                )
                                while (
                                    ("nextPageToken" in response.keys())
...
"""
                            ...
                            print(
                                "  Все искомые объекты в году",
                                year,
                                "найдены БЕЗ включения некоторых значений аргумента order (в т.ч. вообще БЕЗ них)",
                            )
                        year -= 1
                        if (
                            yearMinByUser != None
                        ):  # если пользователь ограничил временнОй диапазон
                            if (year) <= yearMinByUser:
                                goC = False
                                print(
                                    f"Завершил проход по заданному пользователем временнОму диапазону: {yearMinByUser}-{yearMaxByUser} (с точностью до года)\n"
                                )
...
--- Если НЕ хотите их выгрузить, нажмите пробел и затем Enter. Тогда исполнение скрипта завершится"""
    ...
    if len(itemS) > 0:  # если использовался search..
        if (
            sum(itemS["id.kind"].str.split("#").str[-1] == snippetContentType) > 0
        ):  # .. и в его выдаче есть плейлисты
            playlistIdS = itemS[itemS["id.kind"] == f"youtube#{snippetContentType}"]
            playlistIdS = (
                playlistIdS[f"id.{snippetContentType}Id"].to_list()
                if f"id.{snippetContentType}Id" in playlistIdS.columns
                else playlistIdS["id"].to_list()
            )
    ...
    if len(itemS) > 0:  # если использовался search..
        ...
        if (
            sum(itemS["id.kind"].str.split("#").str[-1] == snippetContentType) > 0
        ):  # .. и в его выдаче есть видео
            print(
                "В скрипте используются следующие аргументы метода",
                method,
                'API YouTube: part=["snippet", "contentDetails", "localizations", "statistics", "status", "topicDetails"], id, maxResults .',
                "Эти аргументы, кроме part, пользователю скрипта лучше не кастомизировать во избежание поломки скрипта.",
                "Если хотите добавить другие аргументы метода",
                method,
                "API YouTube, можете ознакомиться с ними по ссылке: https://developers.google.com/youtube/v3/docs/videos",
            )
            if expiriencedMode == False:
                input("--- После прочтения этой инструкции нажмите Enter")
            print("")  # для отступа
            ...
            videoIdS = (
                videoIdS[f"id.{snippetContentType}Id"].to_list()
                if f"id.{snippetContentType}Id" in videoIdS.columns
                else videoIdS["id"].to_list()
            )
...
--- Если НЕ хотите пополнить список, нажмите пробел и затем Enter"""
        ...
        if len(input()) == 0:
            # Список списков, каждый из которых соответствует одному плейлисту
            playlistVideoId_list = (
                playlistS["snippet.resourceId.videoId"].str.split(", ").to_list()
            )
    ...
    if len(videoIdS) > 0:
        print(
            f"""Проход по видео{" порциями по 50 штук" if len(videoIdS) > 50 else ""} для выгрузки их характеристик (дополнительных к выруженным методом search)"""
        )
        ...
        try:
            ...
            response = (
                ...
                .list(part="snippet", id=uniqueCategorieS)
                .execute()
            )
        ...
        for row in categoryNameS.index:
            videoS.loc[videoS["snippet.categoryId"] == row, "categoryName"] = (
                categoryNameS["snippet.title"][row]
            )
        ...
        if len(input()) == 0:
            ...
            print(
                "В скрипте используются следующие аргументы метода",
                method,
                'API YouTube: part=["snippet", "id", "replies"], maxResults, videoId .',
                "Эти аргументы, кроме part, пользователю скрипта лучше не кастомизировать во избежание поломки скрипта.",
                "Если хотите добавить другие аргументы метода",
                method,
                "API YouTube, можете ознакомиться с ними по ссылке: https://developers.google.com/youtube/v3/docs/commentThreads",
            )
            if expiriencedMode == False:
                input("--- После прочтения этой инструкции нажмите Enter")
            print("")  # для отступа
            ...
            for videoId in tqdm(videoIdS):
                ...
                if problemVideoId != None:
                    problemVideoIdS.append(problemVideoId)
            ...
            commentS = commentS.drop(
                ["kind", "etag", "id", "snippet.channelId", "snippet.videoId"], axis=1
            )  # т.к. дублируются содержательно
            ...
            print("")
            if (
                len(commentS[commentS["snippet.totalReplyCount"] > 0]) > 0
            ):  # есть ли хотя бы один отвеченный родительский (topLevel) комментарий?
                print(
                    "Проход по строкам всех родительских (topLevel) комментариев, имеющих ответы"
                )
                replieS = pandas.DataFrame()
                for row in tqdm(
                    commentS[commentS["snippet.totalReplyCount"] > 0].index
                ):
                    addReplieS = pandas.json_normalize(
                        commentS["replies.comments"][row]
                    )

                    # Записать разницу между ожданиями и реальностью в новый столбец `Недостача_ответов`
                    commentS.loc[row, "Недостача_ответов"] = commentS[
                        "snippet.totalReplyCount"
                    ][row] - len(addReplieS)

                    replieS = pandas.concat([replieS, addReplieS]).reset_index(
                        drop=True
                    )
                ...
                print(
                    "\nВ скрипте используются следующие аргументы метода",
                    method,
                    'API YouTube: part=["snippet", "id"], maxResults, parentId, textFormat .',
                    "Эти аргументы, кроме part, пользователю скрипта лучше не кастомизировать во избежание поломки скрипта.",
                    "Если хотите добавить другие аргументы метода",
                    method,
                    "API YouTube, можете ознакомиться с ними по ссылке: https://developers.google.com/youtube/v3/docs/commentThreads",
                ...
                print(
                    "Проход по id всех родительских (topLevel) комментариев с недостачей ответов для выгрузки этих ответов"
                )
                commentIdS = commentReplieS["id"][
                    commentReplieS["Недостача_ответов"] > 0
                ]
                for commentId in tqdm(commentIdS):
                    page = 0  # номер страницы выдачи
                    repliesAdditional, goS, keyOrder, problemCommentId = (
                        downloadComments(
                            API_keyS=API_keyS,
                            sourceId=commentId,
                            keyOrder=keyOrder,
                    ...
                    if problemCommentId != None:
                        problemCommentIdS.append(problemCommentId)
                ...
                print(
                    "Ответов выгружено",
                    len(replieS),
                    "; проблемные родительские (topLevel) комментарии:",
                    problemCommentIdS
                    if len(problemCommentIdS) > 0
                    else "отсутствуют\n",
                )
                ...
                print(
                    "Нет ни одного откомментированного родительского (topLevel) комментария"
                )
    ...
    print(
        "Скрипт исполнен. Модуль создан при финансовой поддержке Российского научного фонда по гранту 22-28-20473"
    )
    if os.path.exists(rootName):
        print(
            "Поскольку данные, сохранённые при одном из прошлых запусков скрипта в директорию Temporal, успешно использованы, УДАЛЯЮ её во избежание путаницы при следующих запусках скрипта"
