...
def keywordTagTopicURL(dfIn):
    ...
    df = dfIn.copy()
    contentType = (
        "channel" if "brandingSettings.channel.keywords" in df.columns else "video"
    )
    ...
    for var in varS:
        ...
        if var == "brandingSettings.channel.keywords":
            notNanDf[var] = notNanDf[var].str.split(" ")
        notNanDf[var] = notNanDf[var].apply(lambda cellContent: " ".join(cellContent))
        if var == "topicDetails.topicCategories":
            notNanDf["https://en.wikipedia.org/wiki/"] = notNanDf[var].str.replace(
                "https://en.wikipedia.org/wiki/", ""
            )
        else:  # расщепить на отдельные слова и удалить среди них дубликаты
            # print('var:', var) # для отладки
            notNanDf[var] = notNanDf[var].apply(
                lambda cellContent: textPreprocessor.simbolsCleaner(cellContent)
            )
            notNanDf[var] = textPreprocessor.pymystemLemmatizer(notNanDf, var)
            notNanDf[var] = notNanDf[var].apply(
                lambda cellContent: textPreprocessor.stopwordsDropper(cellContent)
            )
        df = pandas.concat([nanDf, notNanDf])

    df["URL"] = (
        f"https://www.youtube.com/{contentType}/" + df["id"]
    )  # столбец с абсолютной ссылкой
